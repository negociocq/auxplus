...
/** Uma movimentação (recarga) de um revendedor no painel UniPlay. */
...
export async function addIptvResellerCredits(
  ...
  const total =
    opts.saleBrl != null && Number.isFinite(Number(opts.saleBrl))
      ? Number(opts.saleBrl)
      : credits * unit;
  // UniPlay: campo `sale` = valor POR CRÉDITO (o painel exige até R$100/crédito).
  // As chamadas passam o TOTAL (R$ da venda) → converte para unitário = total ÷ créditos,
  // senão a UniPlay registra o total ×10 (ex.: 10×85 = 850 em vez de 85).
  const sale = Math.min(
    100,
    Math.max(0.01, Math.round((total / Math.max(1, credits)) * 100) / 100),
  );
